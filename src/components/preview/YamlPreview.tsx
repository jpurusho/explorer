    ...
    return (
      <div>
        ...
        >
          ...
          <span className="text-text-muted text-[var(--font-xs)]">[{data.length} items]</span>
        </div>
            ...
            return (
              <div key={idx}>
                <div style={{ paddingLeft: `${(depth + 1) * 16}px` }} className="flex items-baseline py-[2px]">
                  ...
                  <span className="text-text-muted text-[var(--font-xs)]">
      ...
      return (
        <div>
          ...
          >
            ...
            <span className="text-accent">{keyName}</span>
            <span className="text-text-muted mx-1">:</span>
            <span className="text-text-muted text-[var(--font-xs)]">{`{${entries.length}}`}</span>
          </div>
...
export function YamlPreview({ content }: YamlPreviewProps) {
    ...
    return (
      <div className="h-full overflow-auto pl-6 pr-8 py-4">
        <div className="flex items-center gap-2 text-amber-400 mb-3">
          ...
          <span className="text-[var(--font-sm)]">Parse error: {result.error}</span>
        </div>
        <pre className="text-[var(--font-sm)] text-text-secondary font-mono whitespace-pre-wrap">
          {content}
        </pre>
      </div>
    );
  ...
  return (
    <div className="h-full overflow-auto pl-6 pr-8 py-3 font-mono text-[var(--font-sm)] leading-[1.7]">
